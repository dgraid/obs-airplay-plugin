...
| Date | Decision | Why |
|---|---|---|
| 2026-08-23 | Plugin CPU crossfade + letterbox morph (~300ms) | Hard cuts on connect/lock/unlock/disconnect and on 16:9↔portrait. Snapshot previous source BGRA and contain-rect; lerp pixels and the inner window in `video_tick`. Cover-fill the moving rect; outside the window is alpha 0 (scene shows through), not black bars. OBS source size stays canvas. Do not fade in the helper, do not fade audio, do not delay `airplay_status`. Skip on first paint, canvas resize, stub language refresh, and `Connecting`. |
