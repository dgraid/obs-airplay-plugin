...
| 2026-08-28 | Helper starts on source **create**, stops on **destroy** | Scene switch / Studio preview must not kill Bonjour or the AirPlay session. `deactivate` is a no-op; OBS already skips video/audio from inactive sources. |
| 2026-08-23 | Plugin CPU crossfade + letterbox morph (~300ms) | Hard cuts on connect/lock/unlock/disconnect and on 16:9↔portrait. Snapshot previous source BGRA and contain-rect; lerp the inner window **linearly**, ease only the pixel fade (`smoothstep`). Contain-scale content into that window (16:9 stub grows to 1080 instead of crop-reveal). `obs_source_set_async_unbuffered` so OBS does not drop size frames. Outside the window is alpha 0. OBS source size stays canvas. Do not fade in the helper, do not fade audio, do not delay `airplay_status`. Skip on first paint, canvas resize, stub language refresh, and `Connecting`. Non-live morph (Stop Mirroring / lock) is pumped at ~60 Hz; do not rely on canvas `video_tick` (25 fps looks stepped). |
...
| 2026-08-23 | Idle stub + canvas letterbox in the **plugin**, not helper | Async `obs_source_output_video` already owns pixels. Zoom PNG not used. |
| 2026-08-23 | Source size locked to OBS canvas; live frames contain-scaled with transparent margins | iPhone portrait vs Mac 16:9 must not jump scene-item bounds. Stop mirroring snaps back to stub at the same size, never last native WxH. Bars around the phone are alpha 0, not black. |
...
| 2026-08-23 | Stub language + scene automation are **module** config (Tools window) | Cocoa `NSWindow`, not NSAlert. Canvas `ru`/`en` independent of OBS UI locale. `airplay_status` can switch OBS scenes if names are set. The AirPlay source need not be on both scenes. |
