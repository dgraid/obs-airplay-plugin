...
| Date | Decision | Why |
|---|---|---|
| 2026-08-23 | Dead client = missed `/feedback` ≥8s → Discoverable, not Paused | UxPlay standalone resets after 15s of silence (`feedback_callback`). Our helper had no watchdog: `conn_feedback` was reused as a video-stall→Paused path, and it only runs when a heartbeat **arrives**. Wi-Fi off hangs TCP; last OBS frame stays forever. Restore heartbeat semantics; stall→Paused only if video is silent **and** feedback is still live (<3s). Drop via `raop_remove_known_connections` from the helper loop, not from the mirror thread. |
