...
| 2026-08-23 | Receiver name + stub language are **module** config (Tools) | One Bonjour name. Canvas `ru`/`en` independent of OBS UI locale. Cocoa dialog, no Qt. JSON keeps empty `on_connect_scene` / `on_disconnect_scene` for later automations. |
