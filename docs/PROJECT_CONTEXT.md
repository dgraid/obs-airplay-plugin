...
- `Paused`: session still up (iPhone lock / video stall ≥2.5s **with** live `/feedback`). Lock-specific stub at last iPhone size, letterboxed into the canvas. Identical SPS/PPS on wake does **not** recreate VideoToolbox. `connected` remains true so on-disconnect scene automation does not fire. Unlock / next decoded frame returns to Streaming. Wi-Fi off / vanished client: no `/feedback` for ≥8s → Discoverable (connect stub), `connected=false`.
