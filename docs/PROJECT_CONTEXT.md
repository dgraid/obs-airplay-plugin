...
- `Paused`: session still up (iPhone lock / video stall ≥2.5s). Lock-specific stub at last iPhone size, letterboxed into the canvas. Identical SPS/PPS on wake does **not** recreate VideoToolbox. `connected` remains true so on-disconnect scene automation does not fire. Unlock / next decoded frame returns to Streaming.
