...
- `Paused`: session still up (iPhone lock / video stall ≥2.5s). Stub on canvas. Decoder stays warm so P-frames after unlock can resume. Identical SPS/PPS on wake does **not** recreate VideoToolbox. `connected` remains true so on-disconnect scene automation does not fire. Unlock / next decoded frame returns to Streaming.
